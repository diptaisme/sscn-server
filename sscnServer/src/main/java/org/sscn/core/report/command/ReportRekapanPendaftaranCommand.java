...
@Component("ReportRekapanPendaftaranCommand")
public class ReportRekapanPendaftaranCommand extends ReportCommand {
	...
	protected byte[] generateXls(Object[] pMyData, Map<String, Object> myMap)
		...
		DataRekapanPendaftaranWorkbook wb = new DataRekapanPendaftaranWorkbook(
				myMap, inp);
		wb.createStyles();
		wb.setHeader();
		if (pMyData.length > 0) {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response)
		...
		List<RekapanPendaftaran> listDataRekapan = dtPendaftaranDao
				.getRekapanPendaftaranInstansi(request
						.getParameter("kodeInstansi3"));
		Object[] dataRekapanPendaftaranArray = listDataRekapan.toArray();

		excelMap.put(TEMPLATE, reportBaseDir + "rptRekapanPendaftaran.xls");
		excelMap.put(REPORT_TYPE, "XLS");
		excelMap.put("INSTANSI",
				"Instansi " + request.getParameter("namaInstansi3"));
